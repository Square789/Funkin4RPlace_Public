...
typedef WeekFile =
{
	...
	var ?placePostThumbnail:Null<String>; // f4rp addition
}
...
class WeekData {
	...
	public var placePostThumbnail:Null<String>;
		...
		for (i in 0...directories.length) {
			...
			if (FileSystem.exists(directory)) {
				...
				for (daWeek in listOfWeeks)
				{
					...
					{
						addWeek(daWeek, path, directories[i], i, originalLength, modNames[i], isStoryMode);
					}
				}
				...
				for (file in FileSystem.readDirectory(directory))
				{
					...
					{
						addWeek(file.substr(0, file.length - 5), path, directories[i], i, originalLength, modNames[i], isStoryMode);
					}
				}
			}
		}
	...
	private static function addWeek(weekToCheck:String, path:String, directory:String, i:Int, originalLength:Int, modName:String = '', ?isStoryMode:Bool)
	{
		isStoryMode = isStoryMode ?? PlayState.isStoryMode;
